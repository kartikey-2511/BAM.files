...
C = readRDS("cpm/colorectal.rds")
P = readRDS("cpm/prostate.rds")
...
ui <- fluidPage(
	...
	sidebarLayout(
		...
		sidebarPanel(
			br(),
			h4(strong("Note:"), " The datasets represented here are for", strong(" Colorectal"), " and", strong(" Prostate cancer"), " patients."),
			br(),
			...
			br(),
			h5("For ", strong("Lungs and Pancreas cancer"), "data, visit the following site:"),
			h5(strong(span("https://kartikey25.shinyapps.io/lungs-pancreas-cancer/",style="color:blue"))),
			br(),
			br(),
			selectInput("dataset", h4(strong("Select the cancer tissue dataset")),
						choices = c("Colorectal","Prostate"),
						selected = "Colorectal"),
			br(),
...
server <- function(input, output) {
	...
	tissue <- reactive({
		if (input$dataset == "Colorectal")
			DS = C
		else if (input$dataset == "Prostate")
			DS = P
		return(DS)
	})
	...
	output$name.plot = renderPlot({
		DS = tissue()
		s1 = toupper(input$gene1)
		s2 = toupper(input$gene2)
		r1 = z[s1,1]
		r2 = z[s2,1]
		v1 = DS[r1,]
		v2 = DS[r2,]
	...
	output$id.plot = renderPlot({
		DS = tissue()
		v1 = DS[toupper(input$id1),]
		v2 = DS[toupper(input$id2),]
