...
server <- function(input, output) {
	...
	output$ex.plot2 = renderPlot({
		s1 = plot_x()
		s2 = plot_y()
		x1 = read.table("cpm/n.csv",sep=",",nrows=1,skip=ref[s1,1]-1)
		x2 = read.table("cpm/n.csv",sep=",",nrows=1,skip=ref[s2,1]-1)
		v1 = c(unlist(x1))
		v2 = c(unlist(x2))
	...
	output$cor.plot2 = renderPlot({
		...
		if (l > 0) {
			...
			m = matrix(nrow=l, ncol=length(read.table("cpm/n.csv",sep=",",nrow=1,skip=1)))
			for (i in c(1:l)) {
				s = eid[i]
				x = read.table("cpm/n.csv",sep=",",nrows=1,skip=ref[s,1]-1)
				m[i,] = matrix(unlist(x),nrow=1,ncol=length(x))
			}
			m = t(m)
			colnames(m) = c(goi)
