...
HN = readRDS("cpm/head.and.neck.rds")
...
ui <- fluidPage(
	...
	sidebarLayout(
		...
		sidebarPanel(
			br(),
			h4(strong("Note:"), " The datasets represented here are for", strong(" Colorectal, Head, Neck"), " and", strong(" Prostate cancer"), " patients."),
			br(),
			h5("For", strong("Brain and Lung cancer"), " data, visit the following site:"),
			h5(strong(span("https://kartikey25.shinyapps.io/rse-bl/",style="color:blue"))),
			br(),
			...
			selectInput("dataset", h4(strong("Select the cancer tissue dataset")),
						choices = c("Colorectal","Head and Neck","Prostate"),
						selected = "Colorectal")
		),
...
server <- function(input, output) {
	...
	tissue <- reactive({
		if (input$dataset == "Colorectal")
			DS = C
		else if (input$dataset == "Head and Neck")
			DS = HN
